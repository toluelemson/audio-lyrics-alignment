...
<body>
  <div class="shell">
    <div class="hero">
      ...
      <h1>Upload Once. Reuse Later.</h1>
      <p class="sub">Add a song, paste the lyrics, prepare the timings, build the profile, then run it live from the same song card. Each song keeps its audio, lyrics, timings, and profile in one place.</p>
    </div>
    <div class="grid">
      <section class="card">
        ...
        <form id="addSongForm">
          ...
          <div class="field">
            <label for="audio">Music File (Optional)</label>
  ...
  <script>
    ...
    function renderSongs() {
        ...
        article.innerHTML = `
          <div class="song-top">
            <div>
              ...
              <div class="song-meta">Updated ${new Date(song.updated_at).toLocaleString()}${song.has_audio ? " · with audio" : " · lyrics only / external audio"}</div>
            </div>
            <div class="pill">${song.status}</div>
          </div>
          <div class="song-actions">
            <a href="/songs/${song.song_id}/prepare">Prepare Song</a>
            <a href="/songs/${song.song_id}/lyrics">Edit Lyrics</a>
            <button type="button" data-build-song="${song.song_id}" ${song.can_build_profile ? "" : "disabled"}>${song.has_audio ? "Build Profile" : "Build Profile Needs Audio"}</button>
            <a href="/songs/${song.song_id}/live" ${song.can_run_live ? "" : 'style="pointer-events:none;opacity:0.45"'}>Run Live</a>
      ...
      songsView.querySelectorAll("[data-build-song]").forEach((button) => {
        button.addEventListener("click", async () => {
          try {
            const payload = await api(`/api/songs/${button.dataset.buildSong}/build-profile`, {
              method: "POST",
    ...
    document.getElementById("addSongForm").addEventListener("submit", async (event) => {
      ...
      formData.set("lyrics", document.getElementById("lyrics").value);
      ...
      try {
        ...
        renderSongs();
        event.target.reset();
        setBanner("Song added to the library.");
      } catch (error) {
        setBanner(error.message, "error");
      }
    });
...
<body>
  ...
  <script>
    ...
    function cueSummary(cue) {
      return `${cue.section} · Line ${cue.line_number}`;
    }
    ...
    function render() {
      ...
      cues.forEach((cue, index) => {
        ...
        button.innerHTML = `<div class="list-top"><strong>${cue.section} · Line ${cue.line_number} of ${cue.line_count}</strong><span class="stamp">${stamp}</span></div><div>${cue.lyrics}</div>`;
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .buttons, .operator-row, .jump-row { display: flex; flex-wrap: wrap; gap: 10px; }
...
<body>
  <div class="shell">
    ...
    <p class="sub">Start live tracking for this song without filling raw profile paths. This page is for live lyric control. Clicking a target here jumps the lyric position only. It does not move any audio.</p>
    <div class="grid">
      <section class="card">
        <div class="status">
        ...
        <div class="operator-row" style="margin-top:18px">
        ...
        <div class="jump-row" style="margin-top:14px">
          <input id="jumpSlide" name="jumpSlide" type="number" min="1" placeholder="Slide number">
          <button class="success" type="button" id="jumpButton">Jump Lyrics</button>
        </div>
        <div class="banner" id="banner"></div>
        <div class="label" style="margin:18px 0 10px">Operator Jump Targets</div>
        <div class="caption" style="margin-top:0">Use these only to move the lyric position during live operation. They do not seek or control audio.</div>
  ...
  <script>
    ...
    function render() {
      ...
      slideState.textContent = snapshot && snapshot.last_slide_command ? String(snapshot.last_slide_command.slide_number) : "none";
      ...
      slideTargets.forEach((target) => {
        ...
        const activeSlide = snapshot && snapshot.last_slide_command ? snapshot.last_slide_command.slide_number : null;
        if (activeSlide === target.slide_number) button.classList.add("active");
        button.innerHTML = `<div class="target-meta">Slide ${target.slide_number} · ${target.reference_timestamp.toFixed(2)}s</div><div><strong>${target.section}</strong></div><div>${target.lyrics}</div>`;
        button.addEventListener("click", async () => {
          try {
    ...
    manualOnButton.addEventListener("click", async () => { try { state.payload = await api("/manual/on", { method: "POST", body: "{}" }); setBanner("Manual override enabled."); render(); } catch (error) { setBanner(error.message, "error"); } });
    manualOffButton.addEventListener("click", async () => { try { state.payload = await api("/manual/off", { method: "POST", body: "{}" }); setBanner("Manual override disabled."); render(); } catch (error) { setBanner(error.message, "error"); } });
    manualToggleButton.addEventListener("click", async () => { try { state.payload = await api("/manual/toggle", { method: "POST", body: "{}" }); setBanner("Manual override toggled."); render(); } catch (error) { setBanner(error.message, "error"); } });
    jumpButton.addEventListener("click", async () => {
      const value = Number(document.getElementById("jumpSlide").value);
      if (!Number.isInteger(value) || value <= 0) { setBanner("Jump slide must be a positive integer.", "error"); return; }
      try { state.payload = await api("/jump", { method: "POST", body: JSON.stringify({ slide_number: value }) }); setBanner(`Jumped to slide ${value}.`); render(); } catch (error) { setBanner(error.message, "error"); }
    });
...
def _songs_payload(records: tuple) -> list[dict[str, object]]:
    ...
    for record in records:
        ...
        if has_profile:
            status_label = "Ready for live"
        elif has_timings and has_audio:
            status_label = "Ready to build profile"
        elif has_timings and not has_audio:
            status_label = "Timed, needs audio for profile"
        elif has_audio:
            status_label = "Needs timing"
        else:
            status_label = "Lyrics only"
        payload.append(
            {
...
def main() -> None:
    ...
    from lyrics_aligner.application.click_capture import write_captured_slide_cues
    ...
    def ensure_system_recorder(song_id: str, *, device_id: int | None = None) -> SystemPlaybackRecorder | None:
        if song_library.audio_path(song_id) is not None:
            return None
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_GET(self) -> None:
            ...
            if parsed.path in {"/", "/songs", "/songs/"}:
                self._write_html(
                    LIBRARY_HTML.replace(
                        "__SONGS__",
                        json.dumps(_songs_payload(song_library.list_songs())),
                    )
                )
                return
            ...
            if len(parts) == 3 and parts[0] == "songs" and parts[2] == "lyrics":
                song = song_library.get_song(parts[1])
                lyrics = song_library.lyrics_path(parts[1]).read_text(encoding="utf-8").rstrip()
                page = (
        ...
        def do_POST(self) -> None:
            ...
            try:
                if parsed.path == "/api/songs":
                    ...
                    self._write_json(
                        {"song": {"song_id": record.song_id}, "songs": _songs_payload(song_library.list_songs())},
                        HTTPStatus.OK,
                    )
                    return
                if len(parts) == 4 and parts[0] == "api" and parts[1] == "songs" and parts[3] == "build-profile":
                    ...
                    self._write_json({"songs": _songs_payload(song_library.list_songs())}, HTTPStatus.OK)
                    return
